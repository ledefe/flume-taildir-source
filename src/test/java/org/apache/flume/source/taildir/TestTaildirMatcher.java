...
public class TestTaildirMatcher {
	...
	/**
	 * Append a line to the specified file within tmpDir. If file doesn't exist
	 * it will be created.
	 */
	...
	public void getMatchingFiles() throws Exception {
		...
		TaildirMatcher tm = new TaildirMatcher("f1", tmpDir.getAbsolutePath() + File.separator + "file.*",
	...
	public void testNoMatching() throws Exception {
		TaildirMatcher tm = new TaildirMatcher("nomatch",
				tmpDir.getAbsolutePath() + File.separator + "abracadabra_nonexisting", isCachingNeeded, false);
		List<File> files = tm.getMatchingFiles();
	...
	public void testNonExistingDir() {
		TaildirMatcher tm = new TaildirMatcher("exception", "/abracadabra/doesntexist/.*", isCachingNeeded, false);
	}
	...
	public void testRegexFileNameFiltering() throws IOException {
		...
		TaildirMatcher tm1 = new TaildirMatcher("ab", tmpDir.getAbsolutePath() + File.separator + "[ab].log",
				isCachingNeeded, false);
		// Tail files that starts with c.log
		TaildirMatcher tm2 = new TaildirMatcher("c", tmpDir.getAbsolutePath() + File.separator + "c.log.*",
				isCachingNeeded, false);
