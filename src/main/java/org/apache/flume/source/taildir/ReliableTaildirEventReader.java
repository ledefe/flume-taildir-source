...
public class ReliableTaildirEventReader implements ReliableEventReader {
	...
	private final Table<String, String, String> multilineTable;
	...
	private ReliableTaildirEventReader(Map<String, String> filePaths, Table<String, String, String> headerTable,
			...
			boolean annotateFileName, String fileNameHeader, Table<String, String, String> multilineTable)
			throws IOException {
		// Sanity checks
		...
		for (Entry<String, String> e : filePaths.entrySet()) {
			Map<String, String> multiline = multilineTable.row(e.getKey());
			boolean subDir = false;
			if (multiline.get("subDir") != null) {
				subDir = Boolean.parseBoolean(multiline.get("subDir").trim());
			}
			logger.info("group:" + e.getKey() + ",subDir: " + subDir);
			taildirCache.add(new TaildirMatcher(e.getKey(), e.getValue(), cachePatternMatching, subDir));
		}
		...
		this.multilineTable = multilineTable;
	...
	public List<Long> updateTailFiles(boolean skipToEnd) throws IOException {
		...
		for (TaildirMatcher taildir : taildirCache) {
			...
			Map<String, String> multiline = multilineTable.row(taildir.getFileGroup());
			for (File f : taildir.getMatchingFiles()) {
				...
				if (tf == null) {
					...
					tf = openFile(f, headers, inode, startPos, multiline);
				} else {
					boolean updated = tf.getLastUpdated() < f.lastModified();
					if (updated) {
						if (tf.getRaf() == null) {
							tf = openFile(f, headers, inode, tf.getPos(), multiline);
						}
	...
	private TailFile openFile(File file, Map<String, String> headers, long inode, long pos,
			Map<String, String> multiline) {

		try {
			logger.info("Opening file: " + file + ", inode: " + inode + ", pos: " + pos);
			return new TailFile(file, headers, inode, pos, multiline);
		} catch (IOException e) {
	...
	public static class Builder {
		...
		private Table<String, String, String> multilineTable;
		...
		public ReliableTaildirEventReader build() throws IOException {
			return new ReliableTaildirEventReader(filePaths, headerTable, positionFilePath, skipToEnd, addByteOffset,
					cachePatternMatching, annotateFileName, fileNameHeader, multilineTable);
		}

		public Builder multilineTable(Table<String, String, String> multilineTable) {
			this.multilineTable = multilineTable;
			return this;
		}
